...
%   Copyright 2010 The MathWorks, Inc.
    ...
    methods(Hidden)
        function [varargout] = subsref(this,s)
            ...
            if     strcmp(s(1).type,'()')
                error(message('DisallowVectorOps:subsref:SubscriptReferenceNotAllowed',className));
            elseif strcmp(s(1).type,'{}')
                error(message('DisallowVectorOps:subsref:CellReferenceNotAllowed',className));
            else
        ...
        function [varargout] = subsasgn(this,s,data)
            className = class(this);
            if     strcmp(s(1).type,'()')
                error(message('DisallowVectorOps:subsasgn:SubscriptAssignmentNotAllowed',className));
            elseif strcmp(s(1).type,'{}')
                error(message('DisallowVectorOps:subsasgn:CellAssignmentNotAllowed',className));
            else
        ...
        function throwUndefinedError(this,s)
            error(message('DisallowVectorOps:UndefinedMethod', s, class( this )));
        end
        
        function throwNoCatError(this)
            error(message('DisallowVectorOps:ConcatenationNotAllowed',class(this)));
        end
    end
