function [labels, post_prob]=BCKNNw(PBags,NBags,testBags,Refs,Citers,indicator,w,metric)
...
for i=1:num_testbags
    ...
    for pointer=2:(num_pbags+1)
       ...
       %     temp_weightP=temp_weightP+exp(-temp(index==1)^(2));
%            temp_weightP=temp_weightP+(1/(1+temp(index==1)));
       %     temp_weightP=temp_weightP+(1/(1+temp(index==1)^(2)));
            cp=cp+1;
        end
    end
    for pointer=(num_pbags+2):num_bags
        [temp,index]=sort(dist(pointer,:));
        if(find(index==1)<=Citers+1)
          %  temp_weightN=temp_weightN+exp(-temp(index==1)^(2));
%            temp_weightN=temp_weightN+(1/(1+temp(index==1)));
            %temp_weightN=temp_weightN+(1/(1+temp(index==1)^(2)));
            cn=cn+1;
        end
    end
    
%     postCit=temp_weightP/(temp_weightN+temp_weightP);
%     postCitneg=temp_weightN/(temp_weightN+temp_weightP);
...
%     if (temp_weightP==0)&&(temp_weightN==0) 
%         postCit=0;
%         postCitneg=0;
%     end
...
%         end
%     end
   post_prob=(postCit+postRef)/(postCitneg+postRefneg);
    
    if((postCit+postRef)>(postCitneg+postRefneg))
        labels(i,1)=1;
    else
        if((postCit+postRef)==(postCitneg+postRefneg))
            if(indicator==1)
